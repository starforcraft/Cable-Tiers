...
/**
 * Nearly exact copy of {@link com.refinedmods.refinedstorage.neoforge.storage.diskinterface.DiskInterfaceBakedModel}
 */
public class TieredDiskInterfaceBakedModel extends BakedModelWrapper<BakedModel> {
    ...
    TieredDiskInterfaceBakedModel(final BakedModel baseModel,
    ...
    public List<BakedQuad> getQuads(@Nullable final BlockState state,
        ...
        if (state == null || !(state.getBlock() instanceof TieredDiskInterfaceBlock diskInterfaceBlock)) {
        ...
        final Disk[] disks = extraData.get(ForgeTieredDiskInterfaceBlockEntity.DISKS_PROPERTY);
        if (disks == null) {
            return super.getQuads(state, side, rand);
        }
