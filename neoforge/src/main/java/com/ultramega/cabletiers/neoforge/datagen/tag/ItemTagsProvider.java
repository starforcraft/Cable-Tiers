...
public class ItemTagsProvider extends BlockTagCopyingItemTagProvider {
    ...
    protected void addTags(final HolderLookup.Provider provider) {
        for (final CableTiers tier : CableTiers.values()) {
            this.addAllToTag(tier.getItemTag(CableType.IMPORTER),
                Blocks.INSTANCE.getTieredImporters(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.EXPORTER),
                Blocks.INSTANCE.getTieredExporters(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.DESTRUCTOR),
                Blocks.INSTANCE.getTieredDestructors(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.CONSTRUCTOR),
                Blocks.INSTANCE.getTieredConstructors(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.DISK_INTERFACE),
                Blocks.INSTANCE.getTieredDiskInterfaces(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.AUTOCRAFTER),
                Blocks.INSTANCE.getTieredAutocrafters(tier).values().stream()
                    .map(block -> (Supplier<Item>) block::asItem)
                    .toList());
            this.addAllToTag(tier.getItemTag(CableType.INTERFACE),
