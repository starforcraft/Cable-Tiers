...
public class TieredDestructorNetworkNode extends TieredNetworkNode<TieredDestructorNetworkNode> implements IComparable, IWhitelistBlacklist, IType, ICoverable {
    ...
    private final UpgradeItemHandler upgrades = (UpgradeItemHandler) new UpgradeItemHandler(4, new UpgradeItem.Type[]{UpgradeItem.Type.SPEED, UpgradeItem.Type.SILK_TOUCH, UpgradeItem.Type.FORTUNE_1, UpgradeItem.Type.FORTUNE_2, UpgradeItem.Type.FORTUNE_3}).addListener(new NetworkNodeInventoryListener(this)).addListener((handler, slot, reading) -> tool = createTool());
    ...
    public int getEnergyUsage() {
        return (4 * (RS.SERVER_CONFIG.getDestructor().getUsage() + upgrades.getEnergyUsage())) * getAdditionalEnergyCost();
    }
    ...
    private ItemStack createTool() {
        ItemStack newTool = new ItemStack(getTier() == CableTier.MEGA ? Items.NETHERITE_PICKAXE : Items.DIAMOND_PICKAXE);
