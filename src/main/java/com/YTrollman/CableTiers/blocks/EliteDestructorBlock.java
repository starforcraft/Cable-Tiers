...
public class EliteDestructorBlock extends CableBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new EliteDestructorTileEntity();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!world.isRemote && CollisionUtils.isInBounds(getHeadShape(state), pos, hit.getHitVec())) {
            return NetworkUtils.attemptModify(world, pos, player, () -> NetworkHooks.openGui(
                ...
                new PositionalTileContainerProvider<EliteDestructorTileEntity>(
                    new TranslationTextComponent("gui.refinedstorage.destructor"),
                    (tile, windowId, inventory, p) -> new EliteDestructorContainer(tile, player, windowId),
                    pos
                ),
                pos
            ));
