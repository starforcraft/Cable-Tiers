...
public class UltraImporterBlock extends CableBlock {
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit)
    {
        if(!world.isClientSide && CollisionUtils.isInBounds(this.getLineShape(state), pos, hit.getLocation()))
        {
            ...
            {
                NetworkHooks.openGui((ServerPlayerEntity) player, new PositionalTileContainerProvider<UltraImporterTileEntity>(new TranslationTextComponent(this.getDescriptionId()), (tile, windowId, inventory, p) -> new UltraImporterContainer(windowId, player, tile), pos), pos);
            });
        }
