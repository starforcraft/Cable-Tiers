...
public class UltraConstructorBlock extends CableBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new UltraConstructorTileEntity();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit) {
        if (!world.isRemote && CollisionUtils.isInBounds(getHeadShape(state), pos, hit.getHitVec())) {
            return NetworkUtils.attemptModify(world, pos, player, () -> NetworkHooks.openGui(
                ...
                new PositionalTileContainerProvider<UltraConstructorTileEntity>(
                    new TranslationTextComponent("gui.refinedstorage.constructor"),
                    (tile, windowId, inventory, p) -> new UltraConstructorContainer(tile, player, windowId),
                    pos
                ),
                pos
            ));
