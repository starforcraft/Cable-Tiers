...
public class EliteImporterBlock extends CableBlock {
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit)
    {
        if(!world.isRemote && CollisionUtils.isInBounds(this.getLineShape(state), pos, hit.getHitVec()))
        {
            ...
            {
                NetworkHooks.openGui((ServerPlayerEntity) player, new PositionalTileContainerProvider<EliteImporterTileEntity>(new TranslationTextComponent(this.getTranslationKey()), (tile, windowId, inventory, p) -> new EliteImporterContainer(windowId, player, tile), pos), pos);
            });
        }
