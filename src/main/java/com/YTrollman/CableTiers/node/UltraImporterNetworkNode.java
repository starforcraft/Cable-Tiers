...
public class UltraImporterNetworkNode extends NetworkNode implements IComparable, IWhitelistBlacklist, IType {
    public static final ResourceLocation ID = new ResourceLocation(CableTiers.MOD_ID, "ultra_importer");
    ...
    private int faster = CableConfig.ULTRA_IMPORTER_SPEED.get();
    
    private final BaseItemHandler itemFilters = new BaseItemHandler(36).addListener(new NetworkNodeInventoryListener(this));
    private final FluidInventory fluidFilters = new FluidInventory(36).addListener(new NetworkNodeFluidInventoryListener(this));
    ...
    public UltraImporterNetworkNode(World world, BlockPos pos)
    {
        super(world, pos);
    }
    ...
    public void update()
    {
        ...
        if (type == IType.ITEMS)
        {
            ...
            if (handler.getSlots() > 0) {
                ...
                if (!IWhitelistBlacklist.acceptsItem(itemFilters, mode, compare, stack)) {
                    currentSlot++;
                } else if (ticks % upgrades.getSpeed() / faster == 0) {
        ...
        else if (type == IType.FLUIDS && ticks % upgrades.getSpeed() / faster == 0)
        {
    ...
    public int getType()
    {
        return world.isClientSide ? UltraImporterTileEntity.TYPE.getValue() : type;
    }
