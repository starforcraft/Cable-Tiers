...
public final class ContentIds {
    private static final Map<CableTiers, Map<CableType, Identifier>> CONTENT_MAP = new EnumMap<>(CableTiers.class);

    private ContentIds() {
    }

    public static Identifier getContentId(final CableTiers tier, final CableType type) {
        return CONTENT_MAP.getOrDefault(tier, Map.of()).get(type);
    }

    static {
        for (final CableTiers tier : CableTiers.values()) {
            final Map<CableType, Identifier> map = new EnumMap<>(CableType.class);
